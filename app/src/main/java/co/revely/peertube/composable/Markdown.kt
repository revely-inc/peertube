...
{
    MDBlockChildren(parser.parse(text))
}
...
//        val paddingBottom = if (paragraph.parent is Document) 8.dp else 0.dp
...
fun MDFencedCodeBlock(fencedCodeBlock: FencedCodeBlock, modifier: Modifier = Modifier) {
    ...
    Box(modifier.padding(start = 8.dp, bottom = padding)) {
        Text(
...
@Composable
fun MDHtmlBlock(htmlBlock: HtmlBlock)
{
    Html(htmlBlock.literal) {}
}
